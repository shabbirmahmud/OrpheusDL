...
try:
    ...
    from .music_downloader import beauty_format_seconds
...
def main():
    """Main entry point for the OrpheusDL CLI."""
    print(r"""
   ____             _                    _____  _      
  / __ \           | |                  |  __ \| |     
 ...
 | |__| | |  | |_) | | | |  __/ |_| \__ \ |__| | |____ 
  \____/|_|  | .__/|_| |_|\___|\__,_|___/_____/|______|
    ...
    help_ = (
        'Use "settings [option]" for orpheus controls (coreupdate, fullupdate, modinstall), "settings [module]'
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="Select a download output path. Default is the provided download path in config/settings.py",
    )
    parser.add_argument(
        "-lr", "--lyrics", default="default", help="Set module to get lyrics from"
    )
    parser.add_argument(
        "-cv", "--covers", default="default", help="Override module to get covers from"
    )
    parser.add_argument(
        "-cr",
        "--credits",
        default="default",
        help="Override module to get credits from",
    )
    parser.add_argument(
        "-sd",
        "--separatedownload",
    ...
    if (
        orpheus_mode == "settings"
    ):  # These should call functions in a separate py file, that does not yet exist
        ...
        if setting == "refresh":
            ...
            # elif modulesetting in [custom settings function list] TODO (here so test can be replaced)
                ...
                raise Exception(
                    f'Unknown setting "{modulesetting}" for module "{setting}"'
                )
        else:
        ...
        if module in orpheus.module_list:
            ...
            )  # TODO: replace with InvalidModuleError
    else:
        path = (
            args.output
            if args.output
            else orpheus.settings["global"]["general"]["download_path"]
        )
        if path[-1] == "/":
            path = path[:-1]  # removes '/' from end if it exists
        ...
        if orpheus_mode == "search" or orpheus_mode == "luckysearch":
            if len(args.arguments) > 3:
                ...
                if modulename in orpheus.module_list:
                    try:
                        ...
                        raise Exception(
                            f"{args.arguments[2].lower()} is not a valid search type! Choose {media_types}"
                        )
                    ...
                    items = module.search(
                        ...
                        limit=(
                            1
                            if lucky_mode
                            else orpheus.settings["global"]["general"]["search_limit"]
                        ),
                    )
                    if len(items) == 0:
                        raise Exception(
                            f"No search results for {query_type.name}: {query}"
                        )
                    ...
                    if lucky_mode:
                        ...
                        for index, item in enumerate(items, start=1):
                            ...
                            additional_details += (
                                f"[{beauty_format_seconds(item.duration)}] "
                                if item.duration
                                else ""
                            )
                            additional_details += f"[{item.year}] " if item.year else ""
                            additional_details += (
                                " ".join([f"[{i}]" for i in item.additional])
                                if item.additional
                                else ""
                            )
                            if query_type is not DownloadTypeEnum.artist:
                                ...
                                print(
                                    f"{str(index)}. {item.name} - {', '.join(artists)} {additional_details}"
                                )
                            else:
                    ...
                    modules = [
                        i
                        for i in orpheus.module_list
                        if ModuleFlags.hidden not in orpheus.module_settings[i].flags
                    ]
                    raise Exception(
                ...
                print(
                    f"Search must be done as orpheus.py [search/luckysearch] [module] [{media_types}] [query]"
                )
            ...
            if len(args.arguments) > 3:
                ...
                if modulename in orpheus.module_list:
                    ...
                    media_to_download = {
                        modulename: [
                            MediaIdentification(media_type=media_type, media_id=i)
                            for i in args.arguments[3:]
                        ]
                    }
                else:
                    modules = [
            ...
            arguments = (
                tuple(open(args.arguments[0], "r"))
                if len(args.arguments) == 1 and os.path.exists(args.arguments[0])
                else args.arguments
            )
            media_to_download = {}
            for link in arguments:
                if link.startswith("http"):
                    ...
                    for i in orpheus.module_netloc_constants:
                        if re.findall(i, url.netloc):
                            service_name = orpheus.module_netloc_constants[i]
                    if not service_name:
                        raise Exception(
                    ...
                    if (
                        orpheus.module_settings[service_name].url_decoding
                        is ManualEnum.manual
                    ):
                        module = orpheus.load_module(service_name)
                        media_to_download[service_name].append(
                            module.custom_url_parse(link)
                        )
                    else:
                        ...
                        url_constants = orpheus.module_settings[
                            service_name
                        ].url_constants
